...
export async function generateMetadata() {
  try {
    ...
    const categories = [...new Set(videos.flatMap(v => 
      v.meta.category?.split(',').map(c => c.trim()) || []
    ))].join(', ');

    return {
      title: '18+ Free Adult Videos - Watch HD Porn Videos Online',
      description: `Watch free HD adult videos online. Browse our collection of ${videos.length}+ videos in categories like ${categories}. New videos added daily.`,
      keywords: `adult videos, ${categories}, porn videos, free porn, HD porn, streaming porn`,
      openGraph: {
        title: '18+ Free Adult Videos - Watch HD Porn Videos Online',
        description: `Watch free HD adult videos online. Browse our collection of ${videos.length}+ videos in various categories.`,
        images: videos.slice(0, 4).map(video => ({
          url: `https://videodelivery.net/${video.uid}/thumbnails/thumbnail.jpg?time=${Math.floor(video.duration / 2)}s`,
          width: 1280,
          height: 720,
          alt: video.meta.name,
        })),
      ...
      twitter: {
        ...
        title: '18+ Free Adult Videos',
        description: 'Watch free HD adult videos online',
        images: [`https://videodelivery.net/${videos[0]?.uid}/thumbnails/thumbnail.jpg?time=${Math.floor(videos[0]?.duration / 2 || 0)}s`],
      },
    };
  } catch (error) {
    console.error('Error generating metadata:', error);
    return {
      title: '18+ Adult Videos',
      description: 'Watch free HD adult videos online',
    };
  }
}